...
class DroidMemory {
    public:
        ...
        bool operator==(const DroidMemory &ptr) const;
        bool operator!=(const DroidMemory &ptr) const;
        bool operator<(const DroidMemory &ptr) const { return Exp < ptr.Exp; };
        bool operator>(const DroidMemory &ptr) const { return Exp > ptr.Exp; };
        bool operator<=(const DroidMemory &ptr) const { return Exp <= ptr.Exp; };
        bool operator>=(const DroidMemory &ptr) const { return Exp >= ptr.Exp; };
        bool operator==(const size_t size) const { return Exp == size; }
        bool operator!=(const size_t size) const { return Exp != size; }
        bool operator<(const size_t size) const { return Exp < size; }
        bool operator>(const size_t size) const { return Exp > size; }
        bool operator<=(const size_t size) const { return Exp <= size; }
