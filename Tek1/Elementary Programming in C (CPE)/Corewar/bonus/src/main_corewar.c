...
void mallocs_vm(core_t *core)
{
    ...
    for (int i = 0; i < core->nb_champs; i++) {
        ...
        &core->champs[i].instructs[2192], i * 1024, \
        core->champs[i].header.prog_size);
        core->color_txt = copy_champ_color(core->color_txt, i * 1024, \
        core->champs[i], i);
...
void set_values(core_t *core)
{
    core->nb_live = NBR_LIVE;
    core->ctd = CYCLE_TO_DIE;
    for (int i = 0; i < core->nb_champs; i++) {
        core->champs[i].reg[0] = core->champs[i].id;
    }
    mallocs_vm(core);
}
