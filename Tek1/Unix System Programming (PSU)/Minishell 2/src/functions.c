...
void cd_part_two(char **command)
{
    if (chdir(command[1]) == -1) {
        my_putstr(1, command[1]);
        my_putstr(1, ": Not a directory.\n");
    }
}
...
{
    if (!command[1])
        my_putstr(1, "unsetenv: Too few arguments.\n");
    else
...
int my_setenv(char **command, objt_struct_t **head)
{
    ...
    if (my_str_isalpha2(command[1]) == 0 || my_str_isalpha1(command[2]) == 0) {
        my_putstr(1, "setenv: Variable name must contain ");
        my_putstr(1, "alphanumeric characters.\n");
        return (0);
    }
    if (exist_node(command[1], *head) == 1)
