...
char *my_get_str_word(char const *str, int *n, int char_max)
{
    ...
    while (((str[*n] >= 48 && str[*n] <= 57) || (str[*n] >= 65 &&
        ...
        str[*n] == 32 || str[*n] == 35 || str[*n] == 42) && i < char_max) {
        cache[i] = str[*n];
        cache[i + 1] = '\0';
        i++;
        *n = *n + 1;
...
int set_char_max(char const *str, int length)
{
    ...
    for (length = 0; str[length] != '\0'; length++) {
        if ((str[length] >= 65 && str[length] <= 90) || (str[length] >= 48 &&
            ...
            str[length] == 32 || str[length] == 35 || str[length] == 42)
            b++;
        else if (str[length] > 122 || (str[length] < 48 &&
            str[length != 32] && str[length] != 42)) {
            if (b > char_max)
                char_max = b;
            b = 0;
        }
...
char **my_str_to_word_array(char const *str)
{
    ...
    while (str[n] != '\0' && a < nb_str) {
        if ((str[n] < 48 && str[n] != 42 && str[n] != 32) ||
